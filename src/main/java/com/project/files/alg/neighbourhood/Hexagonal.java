...
public class Hexagonal extends Neighbourhood{
    ...
    public int funIf(int i, int j, int el1, int el2, int el3, int el4, int[][] m, int[][] tmpM) {
        ...
        if(m[el1][el3] > 0 && r == 0) map.put(m[el1][el3],0); //lewy gorny
        if(m[el1][el4] > 0 && r == 1) map.put(m[el1][el4],0); //prawy gorny
        if(m[el2][el3] > 0 && r == 1) map.put(m[el2][el3],0); //lewy dolny
        if(m[el2][el4] > 0 && r == 0) map.put(m[el2][el4],0); //prawy dolny
    ...
    public int[] mcFunIf(int i, int j, int el1, int el2, int el3, int el4, int[][] m, boolean isRandomElement) {
        ...
        if (randomNum == 0 && m[el1][el3] != -1) list.add(m[el1][el3]);
        if (randomNum == 1 && m[el1][el4] != -1) list.add(m[el1][el4]);
        if (randomNum == 1 && m[el2][el3] != -1) list.add(m[el2][el3]);
        if (randomNum == 0 && m[el2][el4] != -1) list.add(m[el2][el4]);
